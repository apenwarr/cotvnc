...
- (void)filterInitDone:(FilterReader*)theFilter
{
    ...
    rowSize = ((int)frame.size.width * pixelBits + 7) / 8;
	NSParameterAssert( rowSize <= Z_BUFSIZE );
    size = rowSize * (int)frame.size.height;
...
- (void)setUnzippedData:(NSData*)data
{
    ...
    bytesTransferred += [data length];
#endif
    data = [currentFilter filter:data rows:(unsigned)frame.size.height];
...
- (void)setZippedData:(NSData*)data
{
	...
	if(cntl == rfbTightJpeg) {
		...
		if(cinfo.output_width != (int)frame.size.width || cinfo.output_height != (int)frame.size.height || cinfo.output_components != 3) {
			[connection terminateConnection:[NSString stringWithFormat:@"Tight Encoding: Wrong JPEG data received.\n"]];
			jpeg_destroy_decompress(&cinfo);
			return;
		}
		buffer = malloc(3 * (unsigned)frame.size.width);
