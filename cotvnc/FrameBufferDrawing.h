...
NSLog(@"fill x=%f y=%f w=%f h=%f -> %d\n", aRect.origin.x, aRect.origin.y, aRect.size.width, aRect.size.height, aColor);
    ...
    while(lines--) {
        for(i=(unsigned)aRect.size.width; i; i--) {
            *start++ = aColor;
        }
        start += stride;
    }
...
- (void)putRun:(FrameBufferColor*)fbc ofLength:(int)length at:(CGRect)aRect pixelOffset:(int)offset
{
	...
	width = (unsigned)aRect.size.width - offPixels;
	offLines += (unsigned)aRect.origin.y;
	offPixels += (unsigned)aRect.origin.x;
	start = pixels + (int)(offLines * (unsigned)size.width + offPixels);
    stride = (unsigned)size.width - (unsigned)aRect.size.width;
	if(width > length) {
		width = length;
	}
...
- (void)copyRect:(CGRect)aRect to:(NSPoint)aPoint
{
        ...
        FBColor* src, *dst;
        int lines = (int)aRect.size.height;
        int i;
...
NSLog(@"copy x=%f y=%f w=%f h=%f -> x=%f y=%f\n", aRect.origin.x, aRect.origin.y, aRect.size.width, aRect.size.height, aPoint.x, aPoint.y);
    ...
    copyPixelCount += (int)aRect.size.width * (int)aRect.size.height;
        ...
        if(aPoint.x < aRect.origin.x) {
                line_step = 1;
                src_start_x = (int)aRect.origin.x;
                dst_start_x = (int)aPoint.x;
        } else {
                line_step = -1;
                src_start_x = (int)CGRectGetMaxX(aRect) - 1;
                dst_start_x = (int)aPoint.x + (int)aRect.size.width - 1;
        }
        if(aPoint.y < aRect.origin.y) {
                stride = (line_step > 0) ? (int)size.width - (int)aRect.size.width :
                                           (int)size.width + (int)aRect.size.width;
                src_start_y = (int)aRect.origin.y;
                dst_start_y = (int)aPoint.y;
        } else {
                stride = (line_step > 0) ? -(int)size.width - (int)aRect.size.width :
                                           -(int)size.width + (int)aRect.size.width;
                src_start_y = (int)CGRectGetMaxY(aRect) - 1;
                dst_start_y = (int)aPoint.y + (int)aRect.size.height - 1;
        }
        src = pixels + (int)(src_start_y * (int)size.width) + (int)src_start_x;
        dst = pixels + (int)(dst_start_y * (int)size.width) + (int)dst_start_x;
        while(lines--) {
			for(i=(int)aRect.size.width; i; i--) {
				*dst = *src;
				dst += line_step;
				src += line_step;
			}
...
- (void)putRect:(CGRect)aRect fromTightData:(unsigned char*)data
{
    ...
    #ifdef PINFO
        putRectCount++;
        putPixelCount += (unsigned)aRect.size.width * (unsigned)aRect.size.height;
    #endif
...
#ifdef PINFO
	putRectCount++;
	pubPixelCount += (unsigned)aRect.size.width * (unsigned)aRect.size.height;
#endif
...
#ifdef DEBUG_DRAW
NSLog(@"draw x=%f y=%f w=%f h=%f at x=%f y=%f\n", aRect.origin.x, aRect.origin.y, aRect.size.width, aRect.size.height, aPoint.x, aPoint.y);
#endif
	...
	// Clip r to fit the frame buffer size
    r = aRect;
    if(CGRectGetMaxX(r) >= size.width)
	{
        r.size.width = size.width - r.origin.x;
    }
	
    if(CGRectGetMaxY(r) >= size.height)
	{
	...
	w = (size_t)r.size.width;
	h = (size_t)r.size.height;
...
Synopsis: void NSDrawBitmap(const CGRect *rect, int pixelsWide, int
