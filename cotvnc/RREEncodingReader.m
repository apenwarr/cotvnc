...
{
    unsigned int totalSize = (unsigned)frame.size.width * (unsigned)frame.size.height;
...
- (void)drawRectangles:(NSData*)data
{
    ...
    while(numOfSubRects--) {
        ...
        r.origin.x = (float)ntohs(subRect.x) + frame.origin.x;
        r.origin.y = (float)ntohs(subRect.y) + frame.origin.y;
        r.size.width = (float)ntohs(subRect.w);
        r.size.height = (float)ntohs(subRect.h);
