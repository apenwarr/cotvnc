...
- (void)setUncompressedData:(unsigned char*)data length:(int)length
{
	...
	for(tile.origin.y = frame.origin.y; tile.origin.y < frame.origin.y+frame.size.height; tile.origin.y += (float)TILE_HEIGHT) {
		tile.size.height = MIN((float)TILE_HEIGHT, (frame.origin.y + frame.size.height - tile.origin.y));
		for(tile.origin.x = frame.origin.x; tile.origin.x < frame.origin.x+frame.size.width; tile.origin.x += (float)TILE_WIDTH) {
			tile.size.width = MIN((float)TILE_WIDTH, (frame.origin.x + frame.size.width - tile.origin.x));
			subEncoding = *data++;
//			NSLog(@"Subencoding = %d\n", subEncoding);
			if(subEncoding == 0) {
				// raw pixels
				[frameBuffer putRect:tile fromTightData:data];
				data += (int)(cPixelSize * (int)tile.size.width * (int)tile.size.height);
				continue;
			}
			if(subEncoding == 1) {
			...
			if(subEncoding == 128) {
				y = 0;
				while(y < (int)(tile.size.width * tile.size.height)) {
